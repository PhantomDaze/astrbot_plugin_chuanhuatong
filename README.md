...
- **黑白名单功能**：新增黑白名单模式，支持按会话（群/私聊）控制插件启用状态
  ...
  - `/传话筒开启` - 在当前会话启用传话筒
  - `/传话筒关闭` - 在当前会话禁用传话筒
  - `/传话筒状态` - 查询当前状态（启用状态、模式、配置类型、预设）
  - `/切换预设 <预设名>` - 切换预设（仅当前会话）
  - `/预设列表` - 列出所有可用预设

- **会话独立预设**：支持每个会话（群/私聊）独立配置预设
  - 使用 `/切换预设` 指令可为当前会话单独设置预设
  - WebUI 的"应用配置"仅影响未单独配置的会话
- **权限控制**：可配置控制指令的权限范围
  - `admin` - 仅框架管理员
...
本仓库是在原项目基础上的二次开发分支。与原仓库相比，当前分支主要有以下差异：

- **系统字体回退渲染**
  - 当主字体缺字时，渲染会自动回退到系统字体补齐。
  - 适合 emoji、特殊符号和生僻字，不再完全依赖单一字体文件。

- **人格预设绑定**
  - 新增 `persona_preset_bindings`，可把 AstrBot 人格绑定到某个布局预设。
  - 渲染优先级为：`人格绑定预设 > 全局布局`，用于让不同人格对应不同聊天框样式。

- **WebUI 小改动**
  - WebUI 新增“人格预设绑定”面板，可直接选择人格并绑定到预设。
  - 绑定列表支持解绑操作，减少手动编辑配置文件的需要。
